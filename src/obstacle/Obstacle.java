...
public abstract class Obstacle {
	...
	public boolean placeObstacle(Vector3 position) {
			...
			for(int x = 0; x < xLength; x++) {
				for(int y = 0; y < yLength; y++) {
					for(int z = 0; z < zLength; z++) {
						if(terrain.terrain[position.x + x][position.y + y][position.z + z] != null) {
							return false;
						}
					}
				}
			...
			for(int x = 0; x < xLength; x++) {
				for(int y = 0; y < yLength; y++) {
					for(int z = 0; z < zLength; z++) {
						...
						terrain.terrain[position.x + x][position.y + y][position.z + z].pos1 = new Vector3f(position.x * terrain.cubeSize.x, position.y * terrain.cubeSize.y, position.z * terrain.cubeSize.z);
